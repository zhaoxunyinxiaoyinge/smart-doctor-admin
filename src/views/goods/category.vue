...
import { getCategoryData, postCategoryData, putCategoryData,deleteCategoryData} from "@/views/goods/api/index";
...
const list = reactive([]);
const formField = reactive([{ label: "分类名称", name: "name", type: "input", width: 100 },
...
}, { width: 100, label: "图标", name: "icon", type: 'upload' }]);
...
onMounted(() => {
  ...
  { label: "status", prop: "status", format: (row) => row.status == 1 ? '正在使用' : '未使用' }, { label: "icon", prop: "icon" }, { label: "updateTime", width: 150, prop: "updateTime" }, { label: "createTime", width: 150, prop: "createTime" }
  ]);
})

const getList = async (params) => {
  list.length = 0;
  let res = await getCategoryData({ ...params, page: listQuery.page, pageSize: listQuery.pageSize });
  if (res.code == 1) {
...
<template>
  ...
  <div class="flex-1">
    <el-table ref="multeSelection" stripe :data="list" width="100%" height="100%"
      @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" />
      <template v-for="item in tableConfig">
        <el-table-column v-if="item.label == 'icon'" :key="item.label" :width="item.width" :label="item.label"
          :prop="item.prop">
          <template #default="scope">
            <span v-show-image>
              <el-image :src="scope.row['icon']" style="width:100px" />
            </span>
          </template>
        </el-table-column>

        <el-table-column v-else-if="!item.template" :key="item.label" :formatter="item.format" :width="item.width"
  ...
  <Pagetions :current-page="listQuery.page" :page-size="listQuery.pageSize" :total="listQuery.total" @page="handlePage"
    @size="handleSize" />
  <Edit :dialog-visible="dialogVisible" :title="title" :edit-config="editConfig" @close="handleClose"
    :edit-data="editData" @confirm="handleConfirm" />
</template>
